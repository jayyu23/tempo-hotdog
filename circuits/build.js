...
async function build() {
  ...
  console.log('\n6. Copying artifacts to public/zk...')
  ...
  fs.copyFileSync(wasmSrc, path.join(PUBLIC_ZK_DIR, `${CIRCUIT_NAME}.wasm`))

  // Copy zkey
  fs.copyFileSync(zkeyPath, path.join(PUBLIC_ZK_DIR, `${CIRCUIT_NAME}.zkey`))

  // Copy verification key
  fs.copyFileSync(vkeyPath, path.join(PUBLIC_ZK_DIR, 'verification_key.json'))

  console.log('\n✅ Build complete!')
  console.log(`   WASM:  ${path.join(PUBLIC_ZK_DIR, `${CIRCUIT_NAME}.wasm`)}`)
  console.log(`   ZKey:  ${path.join(PUBLIC_ZK_DIR, `${CIRCUIT_NAME}.zkey`)}`)
  console.log(`   VKey:  ${path.join(PUBLIC_ZK_DIR, 'verification_key.json')}`)
  console.log('\n   The app will automatically use real proofs when these files exist.')
}
